...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    for year_dir in sorted(get_paths_matching_regex(AOC_DIR, YEAR_PATTERN),
                           reverse=True):
        ...
        for day_dir in get_paths_matching_regex(year_dir, DAY_PATTERN):
            ...
            solutions = [
                solution.relative_to(AOC_DIR) for solution in solutions
            ]
...
main_font = functools.partial(get_font,
                              path=AOC_TILES_SCRIPT_DIR /
                              "fonts/PaytoneOne.ttf")
secondary_font = functools.partial(get_font,
                                   path=AOC_TILES_SCRIPT_DIR /
...
def get_extension_to_colors():
    ...
    with open(GITHUB_LANGUAGES_PATH) as file:
        ...
        for language, data in github_languages.items():
            if "color" in data and "extensions" in data and data[
                    "type"] == "programming":
...
def parse_leaderboard(leaderboard_path: Path) -> dict[int, DayScores]:
    ...
    with open(leaderboard_path) as file:
        ...
        if no_stars in html:
            return {}
        matches = re.findall(rf"{start}(.*?){end}", html,
...
def request_leaderboard(year: int) -> dict[int, DayScores]:
    ...
    with open(SESSION_COOKIE_PATH) as cookie_file:
        ...
        data = requests.get(PERSONAL_LEADERBOARD_URL.format(year=year),
                            cookies={
...
def get_alternating_background(languages,
                               both_parts_completed=True,
                               *,
                               stripe_width=20):
    colors = [
        ...
        for x in range(image.width):
            for y in range(image.height):
                if fill_only_half and x / image.width + y / image.height > 1:
                    continue
                image.load()[x, y] = colors[((x + y) // stripe_width) %
                                            len(colors)]

    fill_with_colors([NOT_COMPLETED_COLOR,
                      darker_color(NOT_COMPLETED_COLOR)], False)
    if colors:
...
def draw_star(drawer: ImageDraw,
              at: tuple[int, int],
              size=9,
              color="#ffff0022",
              num_points=5):
...
def generate_day_tile_image(day: str, year: str, languages: list[str],
                            day_scores: DayScores | None) -> Path:
    ...
    for part in (1, 2):
        ...
        time, rank = getattr(day_scores, f"time{part}",
                             None), getattr(day_scores, f"rank{part}", None)
        if day_scores is not None and time is not None:
            ...
            if SHOW_CHECKMARK_INSTEAD_OF_TIME_RANK:
                ...
                drawer.line((160, 35 + y, 180, 15 + y),
                            ...
                            width=2)
                continue
            drawer.text((105, 25 + y),
                        "time",
                        fill=font_color,
                        align="right",
                        font=secondary_font(10))
            drawer.text((105, 35 + y),
                        "rank",
                        fill=font_color,
                        align="right",
                        font=secondary_font(10))
...
def handle_day(day: int, year: int, solutions: list[str], html: HTML,
    ...
    if DEBUG:
        if day == 25:
            languages = []
    day_graphic_path = generate_day_tile_image(f"{day:02}", f"{year:04}",
                                               languages, day_scores)
    day_graphic_path = Path('../assets') / day_graphic_path.relative_to(
        AOC_DIR)
    with html.tag("a",
                  href=str(solution_link)
                  if solution_link else "https://adventofcode.com/2022"):
        html.tag("img",
                 closing=False,
                 src=str(day_graphic_path),
                 width=TILE_WIDTH_PX)
...
def fill_empty_days_in_dict(day_to_solutions: dict[int, list[str]],
                            max_day) -> None:
...
def handle_year(year: int, day_to_solutions: dict[int, list[str]]):
    ...
    for day, solutions in day_to_solutions.items():
        handle_day(
            day, year,
            sorted(solutions, key=lambda x: (not x.endswith('2.ts'), x)), html,
            leaderboard.get(day, None))
...
def main():
    for year, day_to_solutions_list in get_solution_paths_dict_for_years(
    ).items():
