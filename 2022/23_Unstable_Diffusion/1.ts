...
[...new Array(10).keys()].forEach(round);

// render({ elves });

const minX = elves.reduce((acc, e) => Math.min(acc, e.x), elves[0].x);
const maxX = elves.reduce((acc, e) => Math.max(acc, e.x), elves[0].x);
const minY = elves.reduce((acc, e) => Math.min(acc, e.y), elves[0].y);
const maxY = elves.reduce((acc, e) => Math.max(acc, e.y), elves[0].y);
